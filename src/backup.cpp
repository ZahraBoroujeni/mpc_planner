...
int main(int argc, char **argv)
{
    ...
    ros::init(argc, argv, "fmpc_sim");
    ros::NodeHandle nodeh;
    //int nsteps = 40;
    int obtacle_count=1;
    int n=3+4*obtacle_count;
    double x1_[]={100,0,4,-10,100,0,0};
...
//      path_pub=nodeh.advertise<nav_msgs::Path>("/path_fmpc", 1000);
//      while (ros::ok()) {
//          ros::spinOnce();
...
void auto_fmpc(int n,double *x0_)
{
 ...
 /* problem setup */
    ...
    int i, j,m, nz, T, niters, nsteps, info;
    double kappa;
...
//    free(At); free(Bt); free(eyen); free(eyem);free(Xhist); free(Uhist);
